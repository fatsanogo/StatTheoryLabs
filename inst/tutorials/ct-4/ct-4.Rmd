...
These distributions will be very useful to you as we progress in the course. It is important to understand their properties to know how to use them. The best way to learn about them is to generate data from them with different values for the parameters and plot their distributions to notice their similarities and differences. 
...
When we are working quickly it can be nice to make simple plots, but we will want more professional looking graphs for the . You need to load the `tidyverse` package before using this functionality. 
