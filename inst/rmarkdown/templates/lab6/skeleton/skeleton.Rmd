...
d.) Now consider the code chunk below. Write a code comment for each of the four lines below explaining what is happening.

```{r}
bodymass_female_all<-subset(pen_sub, sex != "male") 
bodymass_female <- bodymass_female_all$body_mass_g 

bodymass_male_all <- subset(pen_sub, sex != "female")
bodymass_male <- bodymass_male_all$body_mass_g
```
e.) Determine whether or not it is reasonable to assume that the variation in body mass for female and male penguins is the same using $\alpha = 0.02$.

```{r}

```

f.) Is it reasonable to assume that there is a difference in the average body mass of female and male penguins? Make your conclusion using $\alpha = 0.02$.
